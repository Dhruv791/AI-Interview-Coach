...
import { Sun, Moon, Monitor, ChevronDown, User, Settings, LogOut, Menu, Sparkles } from 'lucide-react'
...
export function Topbar({ sidebarCollapsed, onMobileMenuClick }: TopbarProps) {
  ...
  const displayName = user?.full_name ?? (user?.is_guest ? 'Guest' : user?.email ?? 'User')
  ...
  const interviewsLeft = user?.interviews_remaining ?? 2
  ...
  return (
    ...
    >
      ...
      {user?.is_guest && (
        <div className="flex items-center gap-2 bg-gradient-to-r from-amber-500/15 via-amber-500/10 to-primary/15 border border-amber-500/30 rounded-full px-3 py-1 text-xs shadow-sm">
          <Sparkles className="w-3.5 h-3.5 text-amber-400 animate-pulse" />
          <span className="text-amber-200 font-medium hidden sm:inline">
            Guest Mode • <span className="font-bold text-amber-300">{interviewsLeft}</span> mock interviews left
          </span>
          <span className="text-amber-200 font-medium sm:hidden">
            Guest ({interviewsLeft} left)
          </span>
          <button
            onClick={() => navigate('/register')}
            className="ml-1 bg-amber-400 hover:bg-amber-300 text-slate-950 font-bold text-[10px] uppercase tracking-wider px-2.5 py-0.5 rounded-full transition-all shadow-sm"
          >
            Sign Up
          </button>
        </div>
      ...
      <div className="relative" ref={avatarRef}>
        ...
        >
          <div className={clsx(
            ...
            user?.is_guest ? "bg-gradient-to-br from-amber-500 to-primary shadow-glow-amber/25" : "bg-primary shadow-glow-primary/25"
          )}>
            {initials}
          </div>
        ...
        {avatarOpen && (
          <div className="absolute right-0 top-full mt-2 w-52 bg-slate-900/90 backdrop-blur-md border border-white/5 rounded-2xl shadow-xl shadow-black/50 py-1 z-50 animate-fadeIn">
            <div className="px-3 py-2 border-b border-white/5">
              <p className="text-xs font-bold text-slate-200">{displayName}</p>
              <p className="text-[10px] text-slate-400 truncate">
                {user?.is_guest ? 'Temporary Guest Session' : user?.email}
              </p>
            </div>
            {user?.is_guest && (
              <button
                onClick={() => { navigate('/register'); setAvatarOpen(false) }}
                className="w-full flex items-center gap-2.5 px-3 py-2 text-xs font-bold text-amber-300 bg-amber-500/10 hover:bg-amber-500/20 transition text-left"
              >
                <Sparkles className="w-3.5 h-3.5 text-amber-400" /> Save Progress (Register)
              </button>
            )}
            <button
